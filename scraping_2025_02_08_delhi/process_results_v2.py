...
df = pd.read_csv('/home/ubuntu/work/spotify_management_redux/scraping_2025_02_08_delhi/data/all_candidates_data_09_05_PM.csv')

# Create a new dataframe with unique seats
result_df = df[['seat_id', 'seat_name']].drop_duplicates()

# Calculate AAP votes
aap_df = df[df['cand_party_short'] == 'AAP'].groupby('seat_id')['cand_votes'].sum().reset_index(name='AAP_votes')

# Calculate CONG votes
cong_df = df[df['cand_party_short'] == 'CONG'].groupby('seat_id')['cand_votes'].sum().reset_index(name='CONG_votes')
...
result_df = (result_df
			 .merge(aap_df, on='seat_id', how='left')
			 .merge(cong_df, on='seat_id', how='left')
...
# Save the new CSV
output_path = '/home/ubuntu/work/spotify_management_redux/scraping_2025_02_08_delhi/data/seat_wise_analysis_v2.csv'
